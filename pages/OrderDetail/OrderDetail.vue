<template>
	<view class="container" v-if="details">
					...
					<view class="goods-list-item-content">
						...
						<view class="goods-list-item-content-else">
							<view class="goods-list-item-content-else-price">
								...
								<view class="goods-list-item-content-else-price-old">
									￥{{item.market_price}}
								</view>
							</view>
		...
		<view class="item" v-if="details.status==2||details.status==3||details.status==4||details.status==7">
			<text>支付方式</text>
			<view class="item-color">{{details.payment==1?'支付宝支付':'微信支付'}}</view>
		</view>
		<view class="item" v-if="details.status==11">
			<text>取消原因</text>
			<view>{{details.cancel_order_reason}}</view>
		</view>
		<view class="item" v-if="details.refund_status==7||details.refund_status==8||details.refund_status==9">
			<text>申请退款原因</text>
			<view>{{details.apply_refund_reason}}</view>
		</view>
		<view class="item" v-if="details.who_cancel_refund!=='0'&&details.who_cancel_refund">
			<text>退款状态</text>
			<view class="item-color">{{details.shop_attitude==2?'商家拒绝退款':'商家同意退款'}}</view>
		</view>
		<view class="item" v-if="details.who_cancel_refund==='0'&&details.who_cancel_refund">
			<text>退款状态</text>
			<view class="item-color">用户取消退款申请</view>
		</view>
		<view class="item" v-if="details.who_cancel_refund!=='0'&&details.who_cancel_refund&&details.shop_attitude==2&&details.refund_status==9">
			<text>拒绝原因</text>
			<view>{{details.cancel_refund_reason}}</view>
		</view>
		<view class="item" v-if="details.shop_attitude==1&&details.refund_status==8">
			<text>退款金额</text>
			<view class="item-color">{{details.refund_itg_amounts}}</view>
		</view>
		<view class="item" v-if="details.shop_attitude==1&&details.refund_status==8">
			<text>退款账号</text>
			<view>原支付账号</view>
		...
		<view class="item">
			<text>优惠券</text>
			<view>-￥{{details.coupon_amounts}}</view>
		</view>
		<view class="item">
			<text>运费</text>
		...
		<!-- 按钮 -->
		<view class="btn" v-if="details.status!=11">
			<view class="btn-border" @click="showCancelOrder" v-if="details.status==1">
				取消订单
			</view>
			<view class="btn-bg" @click="toPay" v-if="details.status==1">
				立即支付
			</view>
			<view class="btn-bg" @click="bindRefund" v-if="details.status==2||details.status==4">
				申请退款
			</view>
			<view class="btn-border" @click="bindRefund" v-if="details.status==3">
				申请退款
			</view>
			<view class="btn-border" style="margin-left: 20rpx;" @click="toLogistics" v-if="details.status==3">
				查看物流
			</view>
			<view class="btn-bg" @click="bindReceivingGoods" v-if="details.status==3">
				确认收货
			</view>
			<view class="btn-bg" @click="cancelRefund" v-if="details.status==7&&details.shop_attitude!=1">
				取消退款
			</view>
		</view>
		<!-- 取消订单弹窗 -->
...
<script>
	...
	export default {
		...
		methods:{
			...
			toLogistics() {
				uni.navigateTo({
					url:'/pages/Logistics/Logistics?order_id=' + this.order_id
				})
			},
			// 立即支付
			toPay() {
				...
				uni.navigateTo({
					url: '/pages/Pay/Pay?order_id=' + this.order_id + '&type=' + 3
				})
			},
			// 刷新 上个页面 订单列表
			prevPageRefresh() {
			...
			bindRefund() {
				uni.navigateTo({
					url:'/pages/Regund/Regund?order_id=' + this.order_id + '&type=' + 2
				})
			},
			// 取消退款
			cancelRefund() {
